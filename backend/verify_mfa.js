app.post('/verify-mfa',requireAuth,async(req,res)=>{
    ...
    WHERE user_id = ? AND is_enabled = FALSE`,[process.env.AES_KEY, userId]); //we're creating a list for taking the encrypted password and we solve it
    ...
    const bycrpt = require('bcrypt'); //we're gonna create recovery codes 10 times and record them into database
    const codes = [];
    for(let i = 0;i<10;i++){
        const rawcode = Math.random().toString(36).substring(2, 10).toUpperCase();
        const hashed = await bycrpt.hash(rawcode,10);
        await db.query(
